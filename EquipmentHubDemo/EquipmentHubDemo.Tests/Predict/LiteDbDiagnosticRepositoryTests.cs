...
public sealed class LiteDbDiagnosticRepositoryTests : IDisposable
{
    ...
    [Fact]
    public async Task RepositoryRecoversFromCorruptedFile()
    {
        var dbPath = Path.Combine(_rootPath, "diagnostics", "corrupted.db");
        Directory.CreateDirectory(Path.GetDirectoryName(dbPath)!);

        // Simulate a corrupted LiteDB file by writing plain text to the path the repository will open.
        await File.WriteAllTextAsync(dbPath, "not-a-lite-db");

        using var repository = new LiteDbDiagnosticRepository(dbPath);

        var now = DateTime.UtcNow;
        await repository.AddAsync(new DiagnosticSample("IN-03", "Humidity", 48.2, now));

        var samples = await repository.GetRecentAsync("IN-03", "Humidity", TimeSpan.FromMinutes(5), CancellationToken.None);

        Assert.Single(samples);
        Assert.Equal(48.2, samples[0].Value, 3);
    }
